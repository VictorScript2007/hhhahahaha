...
--// PIXEL-PERFECT BLOX FRUITS TOAST REPLICA (Fixed Black Flash, Highly Saturated, Compact Layout)
local function showSentGiftToast(rawItemName)
    ...
    -- High-Saturation Color Palettes (Rich Vivid Gold and Vibrant Mint Green)
    local mainGoldHex = "rgb(235,180,20)"
    local highlightGoldHex = "rgb(255,210,40)"
    local pureVibrantGreen = Color3.fromRGB(40, 245, 110)
    local shadowColor = Color3.fromRGB(12, 12, 12)
    
    --// HELPER TO CONSTRUCT IMMACULATE COMPACT CHARR TEXT
    local function createCleanLabel(isSuccess)
        local label = Instance.new("TextLabel", toastGui)
        label.Size = UDim2.new(0, 800, 0, 32) -- Reduced frame height to prevent excess sizing
        label.AnchorPoint = Vector2.new(0.5, 0)
        label.Position = UDim2.new(0.5, 0, 0, -100)
        label.BackgroundTransparency = 1
        
        -- Text Presentation Fixes
        label.Font = Enum.Font.GothamBold
        label.TextSize = 27 -- Reduced slightly from 29 for a sharper, compact and tight fit
        label.RichText = true
        label.TextWrapped = false
        
        -- Sharp Shadow Construction (Eliminates the multi-layer delayed flash)
        label.TextStrokeTransparency = 0
        label.TextStrokeColor3 = shadowColor
        
        -- Native Roblox Shadow Properties utilized for a clean 1-2px back offset offset
        local stroke = Instance.new("UIStroke", label)
        stroke.Thickness = 2
        stroke.Color = shadowColor
        stroke.JoinMode = Enum.LineJoinMode.Miter
        
        if isSuccess then
            label.TextColor3 = pureVibrantGreen
            label.Text = "Gift sent successfully!"
        else
            label.TextColor3 = Color3.fromRGB(255, 255, 255)
            label.Text = "Sending gift <font color='".. mainGoldHex .."'>&lt;</font><font color='".. highlightGoldHex .."'>Permanent " .. cleanItemName .. "</font><font color='".. mainGoldHex .."'>&gt;</font> to " .. targetName .. "..."
        end
        
        return label
    end

    local sendingLabel = createCleanLabel(false)
    local successLabel = createCleanLabel(true)

    --// TIMING CONFIGURATION
    local sendingVisibleTime = 2.5 
    local successVisibleTime = 3.0 
    local successDropDelay = math.random(30, 130) / 100 
    ...
    -- Sending label exit logic
    task.delay(sendingVisibleTime, function()
        if not sendingLabel or not sendingLabel.Parent then return end
    ...
    task.delay(successDropDelay, function()
        if not successLabel or not successLabel.Parent then return end
        TweenService:Create(successLabel, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Position = UDim2.new(0.5, 0, 0, 92) -- Shifted closer for layout consistency
        }):Play()
...
local function createBuyUI(price, name, image)
    ...
    buyButton.BackgroundColor3 = Color3.fromRGB(35, 66, 153)
    ...
    safetyFill.Size = UDim2.new(1, 0, 1, 0)
    safetyFill.BackgroundColor3 = Color3.fromRGB(63, 105, 255)
    safetyFill.BorderSizePixel = 0
    safetyFill.ZIndex = 1
    Instance.new("UICorner", safetyFill).CornerRadius = UDim.new(0, 8)
